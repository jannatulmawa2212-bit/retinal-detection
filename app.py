...
# =============================================================================
# POOKIE CSS — Soft pastel, cute, animated
# =============================================================================
st.markdown("""
<style>
...
with st.sidebar:
    ...
    🤖 Uses advanced foundation model pretrained on
...
with col_left:
    ...
    <div class='upload-area'>
        <div style='font-size:13px;font-weight:700;color:#9c27b0;
...
if uploaded and analyze_btn:
    ...
    with col_right:
        # Status
        if detected:
            icons_det = ' '.join([ICONS[LABEL_NAMES.index(n)]
                                  for n in detected])
            st.markdown(f"""
            <div class='status-detected'>
                {icons_det} Disease Detected:
                {' · '.join(detected)}
            </div>
            """, unsafe_allow_html=True)
        else:
    ...
    for name, full, prob, color, dark, icon in zip(
            LABEL_NAMES, LABEL_FULL, probs,
            COLORS, COLORS_DARK, ICONS):
        th  = THRESHOLDS[name]
        det = prob >= th
        ...
        with c_a:
            ...
            <div class='disease-card'
                ...
                <div style='display:flex;justify-content:space-between;
                    ...
                    <span style='color:{dark};font-size:13px;
                                 font-weight:800'>
                        {prob:.3f}
                    </span>
                </div>
